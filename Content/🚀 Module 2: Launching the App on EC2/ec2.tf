...
# Get the default VPC (so the security group is created in it)
data "aws_vpc" "defaultvpc" {
  default = true
}

# Find all default subnets in the default VPC
# data "aws_subnets" → A Terraform data source that queries AWS for subnets.

# "defaultvpc_defaults" → The local name for this query (used later in data.aws_subnets.defaultvpc_defaults).

# filter { name = "vpc-id" ... } → Only look for subnets in the default VPC (ID comes from data.aws_vpc.defaultvpc.id).

# # filter {
#     name   = "tag:Name"
#     values = ["default subnet"] # <-- the Name tag value you saw
#   }
# Filter by the Name tag (matches what you see in the console)

# Result: This data source produces a list of subnet IDs that match those filters.
data "aws_subnets" "defaultvpc_defaults" {
  filter {
    name   = "vpc-id"
...
# vpc_id → Associates it with a specific VPC (here, your default VPC from data.aws_vpc.defaultvpc).
...
# --- Simple Apache HTTP Test User Data ---
locals {
  user_data = <<-EOF
    #!/bin/bash
    set -eux
    ...
    <body>
      ...
      <p>This static website was launched with a User Data script on Amazon Linux 2023.</p>
    </body>
    </html>
    EOT
  EOF
...
# What it does:
# data.aws_subnets.defaultvpc_defaults.ids → This is the list of subnet IDs from the previous data source.

# element(list, index) → Terraform function that returns the item at a given position in a list.

# element(..., 0) → Returns the first subnet in that list.

  subnet_id              = element(data.aws_subnets.defaultvpc_defaults.ids, 0) # pick first available subnet

# How they work together
# First step (data "aws_subnets" ...)
# → Ask AWS: “Give me all default subnets in my default VPC.”

# Second step (element(...))
# → From that list, pick the first one and use it when launching the EC2.

# Analogy:

# Step 1: You walk into a parking garage and see all available parking spaces (subnets).

# Step 2: You say “I’ll just take the first spot I see” (element(..., 0)).
